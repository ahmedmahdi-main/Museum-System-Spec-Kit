...
### User Story 2 - Prevent documentation outside Documentation custody (Priority: P1)

Documentation staff can view relevant artifact information, but the system blocks creating or editing documentation when the artifact is not currently available to the Documentation Department according to the existing custody/movement state.
...
**Independent Test**: Search for an artifact that is not in Documentation custody and verify that creation and editing are blocked while registry details remain protected from redefinition.

**Acceptance Scenarios**:

1. **Given** an artifact is in Storehouse custody, **When** a Documentation employee searches for it, **Then** the system shows the artifact's essential registry information and indicates that documentation cannot be created or edited until the artifact is transferred to Documentation.
2. **Given** an artifact has a Completed Documentation Record but is no longer available to Documentation, **When** an employee attempts to correct the completed record, **Then** the system prevents the edit and explains the custody requirement.
...
3. **Given** a template version has been used by at least one Documentation Record, **When** a user attempts to alter or remove it in a way that changes historical meaning, **Then** the system prevents destructive modification and directs the user to create a new version instead.
...
- An artifact exists but is not currently available to the Documentation Department.
...
- A Completed Documentation Record is corrected more than once.
- A user attempts to manage or delete a historical template version already used by records.
...
- Two users attempt to edit the same Draft or Completed Documentation Record at the same time.
- Documentation is completed while the artifact remains in Documentation custody, then is later returned using Storehouse Operations.
...
- **FR-004**: The system MUST verify that an artifact is currently available to the Documentation Department before allowing creation or editing of a Documentation Record.
- **FR-005**: The system MUST allow users to view relevant artifact information while blocking documentation actions when custody requirements are not met.
...
- **FR-012**: The system MUST support saving a Documentation Record as Draft.
- **FR-013**: The system MUST allow authorized Documentation staff to resume and update a Draft Documentation Record.
- **FR-014**: The system MUST allow a Documentation Record to be marked Completed only when all required template fields contain acceptable values.
...
- **FR-019**: The system MUST associate each Documentation Record with exactly one artifact and one exact Documentation Template version.
- **FR-020**: The system MUST preserve metadata for each Documentation Record identifying who created it, when it was created, who last modified it, when it was last modified, who completed it, and when it was completed.
- **FR-021**: Authorized users MUST be able to create a Documentation Template for an artifact category.
- **FR-022**: Authorized users MUST be able to define template fields, reorder fields, mark fields required or optional, select field types, define selectable options, and organize fields into logical sections or groups.
- **FR-023**: Authorized users MUST be able to create a new template version without changing the historical meaning of prior versions.
- **FR-024**: Authorized users MUST be able to activate a Documentation Template version for a category.
- **FR-025**: Authorized users MUST be able to retire an older Documentation Template version.
- **FR-026**: Authorized users MUST be able to view existing Documentation Template versions for a category.
- **FR-027**: Documentation Template versions that have been used by Documentation Records MUST NOT be destructively deleted or modified in a way that changes existing records' historical meaning.
- **FR-028**: New Documentation Records MUST use the currently active template version for the artifact category at the time the record is created.
- **FR-029**: Existing Documentation Records MUST remain bound to the exact template version under which they were created, even after a newer version is activated.
- **FR-030**: The system MUST allow authorized Documentation staff to correct or update a Completed Documentation Record.
- **FR-031**: Every post-completion modification MUST create a new traceable historical revision.
- **FR-032**: The system MUST preserve enough revision history to answer what the documentation previously contained, what changed, who changed it, and when it changed.
- **FR-033**: Authorized users MUST be able to inspect the history of a Documentation Record.
- **FR-034**: Documentation history MUST clearly identify successive revisions with author and timestamp information.
- **FR-035**: The feature MUST provide one coherent Documentation capability and MUST NOT model manual and electronic documentation as separate documentation domains.
- **FR-036**: The feature MUST NOT implement artifact photography, image capture, image upload, image replacement, image deletion, or image management.
- **FR-037**: The feature MUST NOT depend on an unfinished Photography module to complete Documentation workflows.
- **FR-038**: The feature MUST NOT own or reimplement artifact identity, Museum Number generation, artifact category management, Storehouse locations, artifact custody, or artifact movement.
- **FR-039**: The system MUST distinguish Documentation workflow state from physical custody and movement state in all user-facing status and actions.
- **FR-040**: The primary employee workflow MUST minimize duplicate entry by reusing artifact data already maintained in the central artifact registry.
- **FR-041**: The primary employee workflow MUST support the sequence: enter or search Museum Number, view artifact details, open documentation, fill the dynamic form, then Save Draft or Complete.
- **FR-042**: The system MUST distinguish authorization for viewing documentation, creating documentation, editing documentation, completing documentation, viewing documentation history, viewing templates, and managing templates.
- **FR-043**: The feature MUST NOT include artifact creation, core artifact editing, general artifact category administration, Storehouse location management, custody management, movement management, laboratory operations, conservation or maintenance, exhibition management, loans, external archive integration, notifications, general reporting, PDF or Word export, printing, barcode or QR functionality, OCR, artificial intelligence features, authentication redesign, authorization infrastructure redesign, or general audit infrastructure redesign.
- **FR-044**: The system MUST provide clear user feedback when a requested documentation action is blocked by custody, missing template, required field validation, status, or authorization.

### Key Entities *(include if feature involves data)*

- **Documentation Record**: The digital documentation for one artifact, created from one exact Documentation Template version and carrying Draft or Completed status plus creation, modification, completion, and revision metadata.
...
- **Documentation Template Version**: A specific historical version of a Documentation Template that can be active, retired, or retained for historical records; records remain bound to the version used at creation.
...
- **Documentation Revision**: A preserved historical snapshot or change record for a post-completion modification, identifying prior content, changed content, author, and timestamp.
...
- **SC-003**: 100% of attempted documentation creations or edits for artifacts outside Documentation custody are blocked with a clear explanation.
...
- **SC-007**: 100% of post-completion edits tested create a traceable revision identifying previous content, changed content, author, and timestamp.
...
- Template changes after records exist are handled by creating new versions rather than mutating prior used versions.
- Revision history may be represented in whichever durable form planning determines, provided the user-facing history requirements are met.
